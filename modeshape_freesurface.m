function [f]=modeshape_freesurface(u,node,element,order,method)
% 振型图不加变形
% 归一化振型
u = normalize(u,'range');
...
for i=1: size(element,1)
    ...
    dof_z = nu; % 节点的z方向自由度编号 nz = [dof_nz1, dof_nz2, dof_nz3, dof_nz4]
...
% clim([0 1]) % 颜色范围
title(method)
% title([method,' Free surface ', 'order=', num2str(order)],'Interpreter', 'none')
% ylim([0,h+0.1])
% xlim([-0.2,0.2])
yticks([])
xticks([])
