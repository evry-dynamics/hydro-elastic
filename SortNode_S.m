function [nodeS_FSI,DOF_INTER,DOF_EDGE_SORT,DOF_INDENPENT,DOF_A,...
    edge_dof_num,index_L_row,index_B_row,index_BL_row,index_R_row,index_T_row,index_INTER_row,...
    index_BR_row,index_TL_row,index_TR_row,index_L_col,index_B_col,index_BL_col,index_INTER_col] = SortNode_S(node)
...
id_FSI = find( co(:,2)>= ymin );   %找到fsi边界上的节点编号
[~,idx] = sort( co(id_FSI,1) );   %对Top边界上的节点按x进行从小到大排序
id_FSI = id_FSI(idx);             %排序后编号
[~,idx] = sort( co(id_FSI,2) );   %对Top边界上的节点按y进行从小到大排序
id_FSI = id_FSI(idx);             %排序后编号
nodeS_FSI = id_FSI;   
